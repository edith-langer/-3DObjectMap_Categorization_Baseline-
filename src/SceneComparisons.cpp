...
/// input folder of 3d_obj_map.txt
...
/// generated by 3d_obj_map
const std::string fv_path_postfix = "_fv_paths.txt";

//CHANGE HERE
/// association of objects with the same label only counts if dot product is smaller than sim_thr
float sim_thr = 1.0; /// 1 == every association counts
const std::string obj_map_path_postfix = "/baseline_result_ycbv_fv1/3d_obj_map.txt";
//const std::string obj_map_path_postfix = "/baseline_result_fv1/3d_obj_map.txt";
std::string prefix_result_path = "/home/edith/liebnas_mnt/ResultBaseline/YCB_fv1/";
...
int main(int argc, char** argv)
{
    ...
    for (std::string r : rooms) {
        ...
        std::cout << room_path << std::endl;

        std::string base_result_path = prefix_result_path + "/" + r;
    ...
    for (ref_it = ref_objects.begin(); ref_it != ref_objects.end(); ref_it++) {
        ...
        if (curr_it == curr_objects.end()) {
            ...
            if (ref_it->second.size() == 1 && curr_it->second.size() == 1) {
                if (computeObjSimilarity(ref_it->second, curr_it->second, ref_result_path, curr_res_path).empty())
                    continue;
...
std::vector<std::tuple<DetObject, DetObject>> computeObjSimilarity(std::vector<DetObject> ref_obj_vec, std::vector<DetObject> curr_obj_vec,
    ...
    for (ref_obj_it = ref_obj_fv_map.begin(); ref_obj_it != ref_obj_fv_map.end(); ref_obj_it++) {
        ...
        if (curr_best_similarity < sim_thr)
            continue;
